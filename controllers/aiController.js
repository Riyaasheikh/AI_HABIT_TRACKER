...
import { lastNDays, calcStreak, todayKey } from "../utils/dataHelper.js";

// Helper function to aggregate context data for the weekly report builder
...
// @desc    Generate personalized weekly habit execution summaries
// @route   GET /api/ai/weekly
export const weeklyReport = async (req, res) => {
  try {
    ...
    if (!ctx.perHabit.length) {
      return res.json({
        content:
          "You don't have any active habits yet. Create your first habit to start tracking - I'll generate a weekly report.",
      });
    }
    ...
    const userMsg = `Here is the user's habit data for the past 7 days (${ctx.days[0]} to ${ctx.days[6]}):\n${ctx.perHabit
      .map(
        (h) =>
          `- ${h.name} (${h.category}, ${h.frequency}): completed ${h.completedDays} of the past 7 days, targetDays: ${h.targetDays}`
      )
...
// @desc    Suggest new habits based on user goals, schedules, and struggles
...
// @desc    Build a 3-day recovery actionable roadmap if a user breaks a streak
...
// @desc    Interactive context-bound Q&A chat interface concerning performance analytics
  ...
  try {
    ...
    const context = habits
      .map((h) => {
        ...
        return `- ${h.name}: ${hLogs.length} completions in the last 30 days. Weekly breakdown (Sun-Sat counts): [${byDow.join(", ")}]`;
      })
      .join("\n");
...
// @desc    Generate morning motivation text bytes personalized by dynamic trends
